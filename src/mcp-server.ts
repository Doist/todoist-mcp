...
- **quick-add-task**: Create a **single** task from one natural-language string using Todoist's Quick Add syntax (the same parser the Todoist apps use). Inline syntax: due date (\`tomorrow at 5pm\`, recurring \`every Mon\`), priority (\`p1\`–\`p4\`), labels (\`@label\`), project (\`#Project\`), section (\`/Section\`), assignee (\`+name\`/\`+email\`), description (text after \` // \`). Resolves project/section/assignee by **name**, not ID. Use when relaying something the user typed in NL form. Do **not** use for: bulk creation (use add-tasks), tasks needing a deadlineDate or duration (use add-tasks), or when you already have IDs and structured fields (use add-tasks).
